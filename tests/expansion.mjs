...
const cashBeforeMigration=E.player(s).cash,old=structuredClone(s);old.version=2;delete old.films[0].contracts;delete old.films[0].promotions;E.migrateSave(old);assert.equal(old.version,7);assert.equal(E.player(old).cash,cashBeforeMigration);assert.ok(old.films[0].contracts);assert.ok(E.validateSave(JSON.parse(JSON.stringify(old))));
