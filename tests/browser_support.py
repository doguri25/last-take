...
def fixture_payload():
 ...
 return {'html':html,'css':'\n'.join((dist/n).read_text() for n in ['style.css','portrait.css','start-screen.css','experience.css','studio-v12.css','studio-v13.css']),'modules':modules,'assets':{p.name:base64.b64encode(p.read_bytes()).decode() for p in (dist/'assets').glob('*.png')}}
