...
const legacy=structuredClone(s);legacy.version=4;const lf=legacy.films.find(x=>x.id===f.id);delete lf.productionCycles;delete lf.productionCompletedMonth;delete legacy.marketEvents;const cash=E.player(legacy).cash,release=lf.releaseMonth;E.migrateSave(legacy);assert.equal(legacy.version,5);assert.ok(lf.productionCycles[0].inferred);assert.equal(lf.releaseMonth,release);assert.equal(E.player(legacy).cash,cash);assert.ok(E.validateSave(JSON.parse(JSON.stringify(legacy))));
...
assert.equal(V.APP_VERSION,'1.0.2');assert.equal(V.VERSION_HISTORY[0].version,'1.0.2');assert.equal(V.VERSION_HISTORY.length,7);
