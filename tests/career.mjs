...
const balance=E.player(legacy).cash,filmIds=legacy.films.map(f=>f.id),seedBefore=legacy.seed;E.migrateSave(legacy);assert.equal(legacy.version,7);assert.equal(legacy.roster[0].age,PEOPLE[0].age+2);assert.equal(E.player(legacy).cash,balance);assert.deepEqual(legacy.films.map(f=>f.id),filmIds);assert.equal(legacy.seed,seedBefore);assert.ok(E.validateSave(legacy));
