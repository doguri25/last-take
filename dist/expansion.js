...
export function licensePitch(s,id){const ip=IPS.find(x=>x.id===id);if(!ip)throw Error('원작을 선택해 주세요.');const p=E.createPitch(s,undefined,ip.genre);p.title=ip.title;p.original=ip.title;p.synopsis=ip.synopsis;p.license={...ip};s.pitches.unshift(p);return p;}
