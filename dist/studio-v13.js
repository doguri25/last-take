import {offerWeeksLeft} from './license-offers.js';
...
export function studioItems(s,key) {
 ...
 if(key==='active')return films.filter(f=>['production','reshoot'].includes(f.status)||(f.screening?.status==='editing'&&f.screening.decision?.id==='reshoot'));
...
export function createStudioUpdates(ctx) {
 ...
 function licenseList() {
  return dialogFrame('원작 라이선스','원작을 이해하고, 우리 영화의 방향을 정하세요.',
   `<p class="small muted">총 ${X.IPS.length}편 · ${new Set(X.IPS.map(x=>x.medium)).size}종류 가운데 현재 두 편을 제안합니다. ${offerWeeksLeft(state())}주 뒤 교체됩니다. 아래 원작과 저자는 모두 가상 설정이며, 기획해 둔 작품은 교체 뒤에도 유지됩니다.</p><div class="rights-grid mt">${X.currentLicenses(state()).map(ip=>`<article class="card rights-card"><div class="row between">${tags([ip.genre])}<span class="tag outline">${esc(ip.medium)}</span></div><h3>${esc(ip.title)}</h3><p class="rights-byline">${esc(ip.author)} · ${ip.year} · ${esc(ip.format)}</p><p class="rights-tagline">${esc(ip.tagline)}</p><p class="small muted">${esc(ip.synopsis)}</p><div class="rights-terms">선급 ${E.money(ip.fee)} · 제작사 수익의 ${Math.round(ip.share*100)}%</div>${btn('원작 정보 · 계약 조건','rights-info',`data-id="${ip.id}"`,'ghost')}</article>`).join('')}</div>`,btn('닫기','close','','ghost'));
 }
