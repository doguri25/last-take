...
export function estimate(d,s){
  ...
  return{set,writer,runtime:RT.runtimeOf(d),runtimeQuality:RT.runtimeQuality(d),runtimeAdjustment:round(scale.base*mult*(RT.runtimeCostFactor(d)-1)),cast:round(cast+negotiation),negotiation,formats:presentation.cost,formatQuality:presentation.quality,total:round(set+writer+cast+negotiation+presentation.cost),reserve:round((set+writer+cast+negotiation+presentation.cost)*.16),months:scale.months,quality:qualityEstimate(d,s)};
}
...
export function greenlight(s,d,company='c0'){
  ...
  f.formats=C.normalizeFormats(d);f.presentationBudget=e.formats;f.formatQuality=e.formatQuality;f.receptionVersion=2;
