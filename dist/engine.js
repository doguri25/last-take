...
import {initFilm,specialEvent,socialNews,trait} from './expansion.js';
...
export function genreFit(p,genres){return Math.round(mean(genres.map(g=>p.genres.includes(g)?94:48)));}
...
export function qualityEstimate(d,s){
  ...
  return Math.round(clamp(talent*.85+subgenreEffect(d)+(teamAffinity(s,crewIds(d))-50)/15+(cast.filter(p=>trait(p)==='연기파').length*.4)+7+SCALE[d.scale].quality+genreBonus(d.genres)+(mean(cast.map(p=>p.coop))-70)/12+(s?(teamCompatibility(s,crewIds(d))-50)/12+historyEffect(s,crewIds(d)):0),30,94));
}
export function estimate(d,s){
  const scale=SCALE[d.scale];
  const mult=Math.max(...d.genres.map(g=>GENRE[g].cost))*(1+(d.genres.length-1)*.1);
  ...
  const ids=crewIds(d).filter(Boolean);const set=round((d.script.license?.fee??0)+scale.base*mult+(d.script.sequelOf?scale.base*.06:0)),writer=round(d.script.fee*(s?(contractQuote(s,person(s,d.script.writer),ids,d.company??'c0').chemistryFactor*contractQuote(s,person(s,d.script.writer),ids,d.company??'c0').relationshipFactor):1)),cast=round(crew.reduce((n,p)=>n+(s?contractQuote(s,p,ids,d.company??'c0').fee:p.fee),0)*sizeFactor);
  return{set,writer,cast,total:round(set+writer+cast),reserve:round((set+writer+cast)*.16),months:scale.months,quality:qualityEstimate(d,s)};
}
function log(s,title,text,kind='news',filmId){s.messages.unshift({id:++s.messageCounter,month:s.month,title,text,kind,filmId});s.messages=s.messages.slice(0,100);}
function transaction(s,c,amount,description,kind='expense',filmId){
  amount=round(amount);c.cash=round(c.cash+amount);
  if(c.id==='c0'){s.ledger.unshift({id:++s.ledgerCounter,month:s.month,amount,description,kind,filmId});s.ledger=s.ledger.slice(0,500);}
}
...
export function refreshPitches(s){
  if(s.month-s.lastPitchRefresh<2)throw Error(`새 제안은 ${date(s.lastPitchRefresh+2)}에 도착합니다.`);
  s.pitches=Array.from({length:6},()=>createPitch(s));s.lastPitchRefresh=s.month;return s.pitches;
}
export function requestPitch(s,id){
  ...
  log(s,`${p.name} 작가의 새 제안`,`「${pitch.title}」 시놉시스가 도착했습니다.`,'script');return pitch;
}
...
function validateDraft(s,d,c){
  ...
  if(!Array.isArray(d.genres)||d.genres.length<1||d.genres.length>3||new Set(d.genres).size!==d.genres.length||d.genres.some(g=>!GENRE[g])||!d.genres.includes(d.script.genre))throw Error('원작 장르를 포함해 장르를 1~3개 선택해 주세요.');
...
export function greenlight(s,d,company='c0'){
  ...
  f.subgenres=d.subgenres?{...d.subgenres}:{};f.subgenreBonus=subgenreEffect(d);f.socialAffinity=teamAffinity(s,crewIds(f));initFilm(s,f);initTimeline(s,f);productionBonds(s,f,1,'새 작품 계약');
  transaction(s,c,-e.total,`「${f.title}」 제작 계약`,'production',f.id);s.films.push(f);
  if(company==='c0'){s.pitches=s.pitches.filter(p=>p.id!==d.script.id);log(s,'새 영화가 시작됩니다',`「${f.title}」 · ${e.months}개월 제작 · ${money(e.total)} 집행`,'production',f.id);}
  return f;
}
...
export function resolveEvent(s,id,index){
  ...
  f.decisions.push({event:f.pending,index,cost:round(f.spent-beforeSpent),quality:f.quality-beforeQuality,awareness:choice[3],month:s.month});f.pending=null;recordWrap(s,f);productionBonds(s,f,choice[2]>0?2:choice[2]<0?-2:0,'현장 사건 선택');
  if(f.company==='c0')log(s,event[0],`${choice[0]} · 품질 ${choice[2]>=0?'+':''}${choice[2]} · ${money(round(f.spent-beforeSpent))} 비용 변동`,'decision',f.id);
  return f;
}
export function expectedRelease(f){if(f.status==='shelved')return Infinity;return f.releaseMonth??(f.status==='ready'?f.readyMonth+1:f.status==='reshoot'?f.reshootStart+12:f.start+f.months);}
...
function prepareReviews(s,f){
  ...
  const offsets=[random(s)*10-5,random(s)*8-4,random(s)*12-6];
  f.reviews=['스크린노트','시네마 페이퍼','필름뷰'].map((name,i)=>({name,base:Math.round(clamp(f.quality+offsets[i],10,99))}));
  finishTimeline(s,f);productionBonds(s,f,2,'촬영 완료');f.readyMonth=s.month;f.status='ready';
...
export const releaseWait=(s,company='c0')=>Math.max(0,2-(s.month-s.companies.find(c=>c.id===company).lastRelease));
export function releaseFilm(s,id){
  ...
  const c=s.companies.find(c=>c.id===f.company),wait=releaseWait(s,c.id);if(wait)throw Error(`다음 개봉은 ${date(s.month+wait)}부터 가능합니다.`);
  if(f.status==='shelved'){f.storageHistory??=[];f.storageHistory.push({action:'release',month:s.month});}f.poster??=GENRES.findIndex(g=>g.id===f.genres[0])%12;f.status='showing';f.releaseMonth=s.month;c.lastRelease=s.month;invalidateCareer(s);
  for(const o of s.films.filter(o=>o.releaseMonth===s.month&&!o.scoreLocked))o.score=reviewScore(s,o,false);
  if(f.company==='c0')log(s,'오늘 개봉합니다',`「${f.title}」 첫 흥행 성적은 다음 달에 집계됩니다.`,'release',f.id);
  return f;
}
export function shelveFilm(s,id){const f=s.films.find(f=>f.id===id);if(!f||f.company!=='c0'||f.status!=='ready')throw Error('완성된 우리 영화를 창고에 보관할 수 있습니다.');f.status='shelved';f.shelvedMonth=s.month;f.storageHistory??=[];f.storageHistory.push({action:'shelve',month:s.month});log(s,'창고영화로 보관합니다',`「${f.title}」 개봉을 미뤘습니다. 제작 슬롯과 별도 보관료 없이 원하는 시기를 기다릴 수 있습니다.`,'production',f.id);return f;}
export function unshelveFilm(s,id){const f=s.films.find(f=>f.id===id);if(!f||f.company!=='c0'||f.status!=='shelved')throw Error('창고에 보관한 우리 영화를 선택해 주세요.');f.status='ready';f.storageHistory??=[];f.storageHistory.push({action:'restore',month:s.month});log(s,'개봉 준비를 다시 시작합니다',`「${f.title}」 창고에서 꺼냈습니다. 홍보와 개봉 시기를 결정해 주세요.`,'production',f.id);return f;}
export function extendFilm(s,id){
  ...
  transaction(s,player(s),-cost,`「${f.title}」 12개월 추가 제작`,'production',f.id);f.spent=round(f.spent+cost);f.extended=true;f.status='reshoot';f.reviews=[];f.readyMonth=null;f.reshootStart=s.month;initTimeline(s,f);f.productionCycles.push({kind:'additional',start:s.month,crankIn:s.month,crankUp:null,completed:null,inferred:false});
...
export function migrateSave(s){
  ...
  initializeRoster(s,s.version===1);initRelations(s);s.marketEvents??=[];if(!s.marketEvents.length)startMarketEvent(s);s.version=VERSION;
...
function aiOperations(s){
  for(const c of s.companies.slice(1)){
    ...
    const ready=s.films.filter(f=>f.company===c.id&&f.status==='ready'&&f.readyMonth<s.month);
    ...
    if((count===0||random(s)<.27)&&s.films.filter(f=>f.company===c.id&&f.status==='ready').length<2)startAi(s,c);
  }
}
function settleBoxOffice(s){
  ...
  for(const f of films){
    ...
    const base=opening?(40000+1600000*Math.pow(f.score/100,2.5)*SCALE[f.scale].reach*(.52+stars/100)*(1+f.awareness/100))*competition*trend*(f.audienceLuck??1)*franchise*(.93+random(s)*.14):f.lastAudience*clamp(.24+f.score*.0033,.3,.57)*(.88+random(s)*.18)*(0.92+competition*.08)*(1+eventBoost/100);
    const audience=Math.max(0,Math.round(base*(1+(f.publicity??0)/100)));const gross=round(audience/10000),royalty=round(gross*.5*(f.script.license?.share??0)),receipts=round(gross*.5-royalty);f.royalties=round((f.royalties??0)+royalty);
...
export function advanceMonth(s){
  ...
  settleBoxOffice(s);
  for(const c of s.companies){const interest=round(c.debt*.005);if(interest)transaction(s,c,-interest,'은행 대출 이자','interest');transaction(s,c,-.25,'월 운영비','overhead');}
  if(s.month%12===11)awards(s);
  s.month++;
  if(s.month%12===0){const changes=cycleYear(s,random);for(const x of changes)log(s,x.type==='death'?x.name+' · '+x.reason:x.name+'의 은퇴 소식',x.type==='death'?`${x.nextName} 님이 같은 프로필 아이콘으로 6세부터 새 경력을 시작합니다.`:x.pending?'현재 작품을 마친 뒤 은퇴합니다.':'작품 기록은 프로필에 보존됩니다.','career');}
...
export function createGame(name,seed=Date.now()>>>0,logo=0){
  ...
  s.ledger=[{id:++s.ledgerCounter,month:0,amount:150,description:'회사 설립 자본금',kind:'capital'}];
  refreshPitches(s);
...
export function validateSave(s){
  if(!s||![1,2,3,4,VERSION].includes(s.version)||!Number.isInteger(s.month)||s.month<0||!Array.isArray(s.companies)||s.companies.length!==10||!Array.isArray(s.films)||!Array.isArray(s.pitches)||!Array.isArray(s.messages)||!Array.isArray(s.ledger)||!Array.isArray(s.awards)||!Array.isArray(s.marketHistory)||!Array.isArray(s.trends))return false;
  if(s.version===VERSION&&(!Array.isArray(s.roster)||s.roster.length!==1600||new Set(s.roster.map(p=>p.id)).size!==1600||!s.alumni||!Array.isArray(s.lifeEvents)))return false;
