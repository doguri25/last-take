...
let features,updates,navigation;
...
function renderDialog(){navigation?.schedule();const more=document.querySelector('.mobile-nav [data-action="more"]');if(more){more.classList.toggle('active',ui.modal?.type==='more'||!navs.slice(0,4).some(n=>n[0]===ui.view));more.setAttribute('aria-expanded',String(ui.modal?.type==='more'));}const dialog=$('dialog');const focus=document.activeElement?.id,selection=document.activeElement?.selectionStart,wasOpen=dialog.open;const scrollBody=dialog.querySelector('.dialog-body');const ownScroll=scrollBody&&getComputedStyle(scrollBody).overflowY==='auto';const top=dialogScroll.transition(dialogKey(ui),ownScroll?scrollBody.scrollTop:dialog.scrollTop);
  ...
  if(m.type==='more')content=dialogFrame('회사 메뉴',esc(E.player(game).name),`<nav class="nav-modal">${navs.slice(4).map(([v,i,t])=>btn(`${icon(i)} ${t}`,'nav',`data-view="${v}"`,'ghost')).join('')}${btn(`${icon('home')} 저장하고 첫 화면`,'home','','ghost')}${btn(`${icon('bell')} 알림함`,'notifications','','ghost')}${btn(`${icon('help')} 플레이 가이드`,'help','','ghost')}${btn(`버전 기록 · ${V.APP_VERSION}`,'versions','','ghost')}</nav>`,btn('닫기','close','','primary full'));
...
function navigationScreen(){
 ...
 return {home:!game||ui.home,view:ui.view,modal:ui.modal?{...ui.modal}:null,state:structuredClone(state),draftId:ui.draft?.script?.id??null,scroll:{page:window.scrollY,dialog:body?.scrollTop??0,dialogOuter:$('dialog').scrollTop,filters:document.querySelector('#main .filter-controls')?.scrollLeft??0}};
}
...
function restoreNavigation(screen){
 ...
 for(const k of navigationFields){if(screen.state?.[k]!==undefined)ui[k]=structuredClone(screen.state[k]);else if(!['people','step','market','filmTab','businessTab','scriptGenre'].includes(k))delete ui[k];}
