...
import {createGrowthFeatures} from './studio-v15.js';
import {createNavigation,cloneNavigationState} from './navigation.js?v=1.6.0';
...
import {createStudioFeatures} from './studio-v12.js?v=1.6.0';
...
let features,updates,navigation,growth;
...
const plus=n=>`${n>=0?'+':''}${n}`;
const statusLabel=f=>f.screening?.status==='editing'?'시사회 보완 중':({production:'제작 중',reshoot:'추가 제작',ready:'개봉 대기',shelved:'창고 보관',showing:'상영 중',closed:'상영 종료',streaming:'OTT 공개 중'}[f.status]);
...
function persist(){if(game)recordFinance(game);try{localStorage.setItem(SAVE_KEY,JSON.stringify(game));saveError=false;}catch{saveError=true;}if(game)Vault.autoBackup(game).then(()=>{ui.vaultWarning='';}).catch(e=>{ui.vaultWarning=e.message;});}
...
function close(){if(ui.modal?.type==='completion'){acknowledgeCompletion(ui.modal.id,false);return;}if(navigation?.closeModal())return;if(ui.peekStack?.length){features.back();return;}if(ui.modal?.type==='negotiation'){ui.modal={type:ui.pendingSelection?'picker':'wizard'};ui.pendingSelection=null;renderDialog();return;}if(ui.modal?.type==='picker'){ui.modal={type:'wizard'};renderDialog();return;}ui.modal=null;renderDialog();}
function startPage(){return `<div class="start-viewport"><div class="start-page"><header class="start-top"><div class="brand"><span class="brand-mark">L/T</span><div><strong>라스트 테이크</strong><small>FILM STUDIO SIMULATOR</small></div></div><div class="start-tools">${utilityButtons(true)}${btn('저장 관리','growth-storage','','text-btn')}<button class="text-btn" data-action="versions">v${V.APP_VERSION} · 버전 기록</button></div></header><main id="main" class="start-grid"><div class="start-intro"><div class="wordmark" aria-hidden="true">LAST<span>TAKE.</span></div><h1>당신의 영화사가 <br>시작되는 순간.</h1><p>이야기를 고르고, 팀을 모으고,<br>세상에 한 편의 영화를 선보이세요.</p><div class="start-facts"><div><strong>10</strong><small>전체 제작사</small></div><div><strong>1,600</strong><small>영화인</small></div><div><strong>3</strong><small>동시 제작</small></div></div></div>${game?resumeCard():`<form class="setup-card" id="found-form"><div class="setup-step"><span>01</span> YOUR FIRST CREDIT</div><h2>제작사 설립</h2><p>엔딩 크레딧에 올라갈<br>당신의 회사 이름을 정해 주세요.</p>${invalidSave?'<div class="notice warning mt-sm">이전 저장 내용을 읽지 못했습니다. 새 게임을 시작하면 저장 내용이 교체됩니다.</div>':''}<label for="company-name">제작사 이름</label><input id="company-name" name="company" value="모먼트 필름" maxlength="24" required autocomplete="organization" aria-label="제작사 이름"><fieldset class="logo-picker"><legend>제작사 로고</legend>${X.LOGOS.map((mark,i)=>`<label><input type="radio" name="logo" value="${i}" ${i===0?'checked':''}><span>${mark}</span></label>`).join('')}</fieldset><div class="difficulty-setup"><label for="difficulty">게임 난이도</label><select id="difficulty" name="difficulty" aria-describedby="difficulty-hint">${K.DIFFICULTIES.map(x=>`<option value="${x.id}" ${x.id==='normal'?'selected':''}>${x.name}${x.id==='normal'?' · 기본':''}</option>`).join('')}</select><p id="difficulty-hint">${K.DIFFICULTY.normal.description}</p></div><div class="setup-summary"><div><span>시작 자본</span><strong id="difficulty-capital">150억 원</strong></div><div><span>시작 시점</span><strong>2026년 1월</strong></div><div><span>진행 방식</span><strong>카드 선택 · 일주일씩 진행</strong></div></div><button type="submit" class="btn primary full">영화사 설립하기 ${icon('arrow')}</button><p class="save-note">진행 상황은 이 브라우저에 자동 저장됩니다.<br>모든 회사와 인물은 가상 설정입니다.</p></form>`}</main><footer class="start-bottom"><span>시나리오 → 캐스팅 → 제작 → 개봉</span><span>라스트 테이크 · 제작자: 도구리</span></footer></div></div>`;}
const navs=[['studio','studio','제작 스튜디오'],['scripts','script','시나리오 제안'],['talents','people','인물정보'],['market','chart','박스오피스'],['finance','bank','재무 · 은행'],['awards','award','영화제·시상식'],['portfolio','library','필모그래피'],['warehouse','library','창고영화'],['ott','globe','OTT 상영관'],['retired','people','은퇴 인물']];
function shell(){
 const c=E.player(game),pending=E.pendingEvents(game).length;
 return `<div class="shell"><aside class="sidebar"><div class="brand"><span class="brand-mark">L/T</span><div><strong>라스트 테이크</strong><small>FILM STUDIO SIM</small></div></div><div class="nav-label">PRODUCTION DESK</div><nav class="nav" aria-label="주 메뉴">${navs.map(([v,i,t])=>`<button data-action="nav" data-view="${v}" class="${ui.view===v?'active':''}" ${ui.view===v?'aria-current="page"':''}>${icon(i)}${t}${v==='studio'&&pending?`<span class="tag green">${pending}</span>`:''}</button>`).join('')}</nav><div class="sidebar-bottom"><nav class="nav"><button data-action="growth-storage">${icon('library')}저장 관리</button><button data-action="home">${icon('home')}저장하고 첫 화면</button><button data-action="help">${icon('help')}플레이 가이드</button><button data-action="versions">${icon('library')}v${V.APP_VERSION} · 제작자: 도구리</button></nav><div class="studio-identity"><span class="avatar">${X.LOGOS[c.logo??0]}</span><div><strong>${esc(c.name)}</strong><small>독립 제작사 · 대표</small></div></div></div></aside><div class="workspace"><header class="topbar"><div class="topbar-date"><span class="calendar-box">${icon('calendar')}</span><div><strong>${E.weekDate(E.weekOf(game))}</strong><small>${Math.floor(game.month/12)+1}년 차 · 1개월 = 4주</small></div></div><div class="topbar-right">${utilityButtons()}<button class="cash-button" data-action="nav" data-view="finance" aria-label="보유 현금 ${E.money(c.cash)} · 재무 화면"><small>보유 현금</small><strong class="${c.cash<0?'red':''}">${E.money(c.cash)}</strong></button>${btn(`${pending?`현장 결정 ${pending}`:'1주 진행'} ${icon(pending?'bell':'arrow')}`,'advance',ui.advancing?'disabled':'','primary advance-button')}</div></header><main id="main" class="content" data-view="${ui.view}">${saveError?'<div class="notice warning save-warning">자동 저장을 사용할 수 없습니다. 이 창을 닫기 전에 저장 환경을 확인해 주세요.</div>':''}${({studio:studioPage,scripts:scriptsPage,talents:talentsPage,market:marketPage,finance:financePage,awards:awardsPage,portfolio:portfolioPage,warehouse:warehousePage,ott:()=>features.ottPage(),retired:()=>updates.retiredPage()}[ui.view]??studioPage)()}<footer class="page-footer"><span>${esc(c.name)} · ${E.weekOf(game)+1}번째 경영 주</span><span>${saveError?'저장 확인 필요':'자동 저장'} · v${V.APP_VERSION}</span></footer></main></div>${mobileDock(pending)}</div>`;
}

function mobileDock(pending){
 const moreActive=!!ui.modal&&ui.modal.type==='more'||!navs.slice(0,4).some(n=>n[0]===ui.view);
 return `<div class="mobile-dock"><div class="mobile-progress"><span class="mobile-progress-date"><small>제작 · 홍보 · 흥행</small><strong>${E.weekDate(E.weekOf(game))}</strong></span>${btn(`${pending?`현장 결정 ${pending}`:'1주 진행'} ${icon(pending?'bell':'arrow')}`,'advance',ui.advancing?'disabled':'','primary advance-button mobile-advance')}</div><nav class="mobile-nav" aria-label="모바일 메뉴">${navs.slice(0,4).map(([v,i,t],n)=>`<button data-action="nav" data-view="${v}" class="${ui.view===v?'active':''}" ${ui.view===v?'aria-current="page"':''}>${icon(i)}<span>${['스튜디오','시나리오','인물정보','흥행'][n]}</span></button>`).join('')}<button data-action="more" class="${moreActive?'active':''}" aria-label="더보기 메뉴" aria-haspopup="dialog">${icon('more')}<span>더보기</span></button></nav></div>`;
}
let dockNode=null,dockObserver=null;
function syncMobileDock(){
 const node=document.querySelector('.mobile-dock');
 const measure=()=>{const height=node&&getComputedStyle(node).display!=='none'?Math.ceil(node.getBoundingClientRect().height):0;const root=document.documentElement;root.style.setProperty('--mobile-dock-height',height+'px');root.style.setProperty('--catalog-nav-height',height+'px');};
 if(node!==dockNode){dockObserver?.disconnect();dockNode=node;if(node&&typeof ResizeObserver==='function'){dockObserver=new ResizeObserver(()=>{measure();fitCatalog();});dockObserver.observe(node);}}
 measure();
}
...
function scriptsPage(){
 const query=(ui.scriptQuery??'').trim(),all=game.pitches.filter(p=>(ui.scriptGenre==='all'||p.genre===ui.scriptGenre)&&(!query||[p.title,p.synopsis,PERSON[p.writer]?.name].some(x=>x?.includes(query)))),scrolling=mobileScriptList(),layout=scrolling?{columns:1,rows:1,size:Math.max(1,all.length)}:catalogLayout('scripts'),size=layout.size,pages=Math.max(1,Math.ceil(all.length/size));
 ui.scriptPage=E.clamp(ui.scriptPage??0,0,pages-1);const pitches=all.slice(ui.scriptPage*size,(ui.scriptPage+1)*size),canRefresh=E.player(game).cash>=E.PITCH_REFRESH_COST;
 return `<section class="catalog-screen scripts-screen ${scrolling?'scripts-scroll':''}">${pageHead('시나리오 제안','좋은 이야기에서 시작되는 한 편의 영화.',`<div class="catalog-actions">${btn('원작 2편','licenses','','ghost small-btn')}${btn(`${icon('refresh')} 다시 받기 · 0.5억`,'refresh-pitches',canRefresh?'':'disabled','ghost small-btn')}</div>`)}<div class="script-filters"><div class="search-wrap">${icon('search')}<input type="search" id="script-query" aria-label="시나리오 검색" data-field="script-query" placeholder="제목 · 작가 · 줄거리" value="${esc(ui.scriptQuery??'')}"></div><select id="script-genre" aria-label="시나리오 장르" data-field="script-genre"><option value="all">모든 장르</option>${GENRES.map(g=>`<option value="${g.id}" ${ui.scriptGenre===g.id?'selected':''}>${g.name}</option>`).join('')}</select>${btn('작가에게 의뢰','writers','','ghost small-btn')}</div><div class="catalog-caption">${all.length} / ${game.pitches.length}편 · ${scrolling?'아래로 스크롤하여 모든 제안 확인':'상세정보에서 전체 이야기 확인'}</div><div class="catalog-results">${pitches.length?`<div class="script-grid" style="--catalog-columns:${layout.columns};--catalog-rows:${layout.rows}">${pitches.map((p,i)=>`<article class="card script-card"><div class="script-head" ${gStyle(p.genre)}><div class="row between"><span class="script-no">SCRIPT ${String(ui.scriptPage*size+i+1).padStart(2,'0')}</span>${tags([p.genre])}</div><h3>${esc(p.title)}</h3></div><div class="script-body"><div class="writer-line">${avatar(PERSON[p.writer])}<div><strong>${esc(PERSON[p.writer]?.name)}</strong><small>작가</small></div><span class="spacer"></span><span class="score-box">${p.quality}<small>시나리오</small></span></div><p class="script-preview">${esc(p.synopsis)}</p><div class="card-info"><span>계약 <strong>${E.money(p.fee)}</strong></span><span>${!E.available(game,PERSON[p.writer])?'계약 불가':E.busyFilm(game,p.writer)?'제작 중':'계약 가능'}</span></div><div class="script-buttons">${btn('상세정보','script-info',`data-id="${p.id}"`,'ghost small-btn')}${btn('이야기로 기획','pitch',`data-id="${p.id}" ${!E.available(game,PERSON[p.writer])||E.busyFilm(game,p.writer)||E.productionSlots(game).length>=3?'disabled':''}`,'primary small-btn')}</div></div></article>`).join('')}</div>`:'<div class="empty"><h3>조건에 맞는 제안이 없습니다.</h3><p>검색어 또는 장르를 바꿔 주세요.</p></div>'}</div>${scrolling?'':`<div class="pagination">${btn('이전','script-page',`data-delta="-1" ${ui.scriptPage===0?'disabled':''}`,'ghost small-btn')}<span>${ui.scriptPage+1} / ${pages} · ${all.length}편</span>${btn('다음','script-page',`data-delta="1" ${ui.scriptPage===pages-1?'disabled':''}`,'ghost small-btn')}</div>`}</section>`;
}
function scriptInfo(id){const p=game.pitches.find(p=>p.id===id);if(!p)return dialogFrame('시나리오 정보','','현재 목록에 없는 제안입니다.');return dialogFrame(esc(p.title),'시나리오 전체 이야기',`${tags([p.genre])}<p class="mt">${esc(p.synopsis)}</p><div class="row mt">${avatar(PERSON[p.writer])}${btn(esc(PERSON[p.writer]?.name),'person',`data-id="${p.writer}"`,'text-btn')}</div><p class="mt small">시나리오 ${p.quality}점 · 계약료 ${E.money(p.fee)}</p>`,`${btn('목록으로','close','','ghost')}${btn('이 이야기로 기획','pitch',`data-id="${p.id}" ${E.busyFilm(game,p.writer)||!E.available(game,PERSON[p.writer])?'disabled':''}`,'primary')}`);}
...
function peopleToolbar(filter,prefix){
 const compactSort={'skill-desc':'실력↓','skill-asc':'실력↑','star-desc':'인지↓','star-asc':'인지↑','fee-asc':'계약↑','fee-desc':'계약↓','age-asc':'나이↑','age-desc':'나이↓','coop-desc':'협업↓','coop-asc':'협업↑'};
 const select=(key,label,choices)=>`<select id="${prefix}-${key}" aria-label="${label}" title="${label}" data-filter="${prefix}" data-key="${key}">${choices.map(([v,n])=>`<option value="${v}" ${filter[key]===v?'selected':''}>${innerWidth<620&&key==='sort'?(compactSort[v]??n):n}</option>`).join('')}</select>`;
 return `<div class="people-toolbar always-filters"><div class="filter-inline" role="group" aria-label="이름 검색과 항상 표시되는 필터"><div class="search-wrap">${icon('search')}<input id="${prefix}-query" aria-label="인물 이름 검색" type="search" placeholder="이름" value="${esc(filter.query)}" data-filter="${prefix}" data-key="query"></div>${select('sort','인물 정렬',[['skill-desc','실력 높은순'],['skill-asc','실력 낮은순'],['star-desc','인지도 높은순'],['star-asc','인지도 낮은순'],['fee-asc','계약료 낮은순'],['fee-desc','계약료 높은순'],['age-asc','나이 낮은순'],['age-desc','나이 높은순'],['coop-desc','협업 높은순'],['coop-asc','협업 낮은순'],...(prefix==='picker'?[['fit-desc','적합↓'],['fit-asc','적합↑']]:[])])}${select('gender','성별',[['all','성별'],['M','남성'],['F','여성']])}${select('genre','전문 장르',[['all','장르'],...GENRES.map(g=>[g.id,g.name])])}${select('age','연령대',[['all','연령'],...['어린이','성인','중년','노년'].map(x=>[x,x])])}${prefix==='people'?select('status','활동 상태',[['all','상태'],['active','현역'],['retired','은퇴'],['dead','고인']]):''}<label class="check-label" title="현재 섭외 가능한 인물만"><input id="${prefix}-available" type="checkbox" aria-label="섭외 가능만" data-filter="${prefix}" data-key="available" ${filter.available?'checked':''}><span>가능</span></label></div></div>`;
}

function personCard(p,picker=false){const busy=E.busyFilm(game,p.id),selected=picker&&E.crewIds(ui.draft).includes(p.id),fit=picker?E.genreFit(p,ui.draft.genres):null;const markup=`<button class="person-card ${busy?'busy':''} ${selected?'selected':''}" data-action="${picker?'choose-person':'person'}" data-id="${p.id}" ${picker&&(busy||selected||!E.eligible(game,p,ui.draft.genres))?'disabled':''}><span class="row">${avatar(p)}<span><strong class="person-name">${esc(p.name)}<small>${p.gender==='M'?'남':'여'}</small></strong><small class="muted">${ROLE_NAMES[p.role]} · ${p.age}세 · ${E.ageGroup(p)}</small></span></span><span class="row wrap" style="gap:5px">${tags(p.genres)}<span class="tag outline">${X.trait(p)}</span></span><span class="person-stats"><span>실력<b>${p.skill}</b></span><span>인지도<b>${p.star}</b></span><span>${picker?'장르 적합':'협업'}<b class="${picker&&fit>=70?'lime':''}">${picker?fit:p.coop}</b></span></span><span class="person-footer"><small>${selected?'선정 완료':p.status==='dead'?'별세':p.status==='retired'?'은퇴':p.retirementPending?'은퇴 예정':!E.available(game,p)?'성장 중':busy?'다른 작품 제작 중':picker&&!E.eligible(game,p,ui.draft.genres)?'이 작품 배역 불가':'섭외 가능'}</small><b>${E.money(E.contractQuote(game,p).fee)}</b></span></button>`;return growth?growth.wrapPerson(p,markup,picker):markup;}
...
function resultLabel(f){return ['closed','streaming'].includes(f.status)?(E.filmProfit(f)>=0?'흥행 성공':'흥행 실패'):f.releaseMonth!=null?'상영 집계 중':statusLabel(f);}
function personDialog(p){
  ...
  return dialogFrame('영화인 프로필','가상 인물 · 기록은 세대가 바뀌어도 보존됩니다.',`
    ...
    ${abilityPanel(p)}
    <div class="card" style="background:#121719"><div class="row between"><span class="muted small">현재 기준 계약료</span><strong style="font-size:24px">${E.money(quote.fee)}</strong></div><p class="muted small mt-sm">기본 ${E.money(K.marketFee(game,p))} × 경력 배수 ${quote.historyFactor} × 친밀도 배수 ${quote.relationshipFactor}. 실제 계약은 함께할 제작진과의 궁합 및 제작 규모를 더 반영합니다.</p></div>
  ...
  `,`${ui.peekStack?.length?btn('뒤로가기','feature-back','','ghost'):btn('닫기','close','','ghost')}${growth?.favoriteControl(p)??''}${p.role==='writer'?btn('이 작가에게 시놉시스 받기','request-pitch',`data-id="${p.id}" ${busy||!E.available(game,p)?'disabled':''}`,'primary'):btn('시나리오부터 기획하기','nav','data-view="scripts"','primary')}`);
}
function abilityPanel(p){const a=p.abilityState,delta=abilityChanges(p);if(!a)return '';return `<div class="ability-summary"><span>기록 시작 이후 ${['skill','star','coop'].map((k,i)=>`${['실력','인지도','협업'][i]} ${plus(delta[k])}`).join(' · ')}</span>${btn('능력 변화 기록','ability-history',`data-id="${p.id}"`,'ghost small-btn')}</div>`;}
function abilityHistory(id){const p=PERSON[id],a=p?.abilityState;if(!a)return dialogFrame('능력 변화 기록','','기록이 없습니다.');const records=a.history,metrics=[['skill','실력'],['star','인지도'],['coop','협업']],from=a.base;const line=k=>records.map((r,i)=>`${i?'L':'M'}${20+i*520/Math.max(1,records.length-1)},${145-r[k]*1.25}`).join(' ');return dialogFrame(`${esc(p.name)} · 능력 변화`,'촬영 경험·연습·휴식·공개작 반응을 매주 반영합니다.',`<p class="small muted">${E.weekDate(a.startWeek)}부터 기록했습니다. 변화가 있었던 최근 32건을 보존하며, 과거 영화의 계약 기록은 바꾸지 않습니다. 연령만으로 실력이 일괄 감소하지 않습니다.</p><div class="ability-legend">${metrics.map(([k,label])=>`<span class="ability-${k}">${label} ${from[k]} → ${p[k]}</span>`).join('')}</div><svg class="ability-chart" viewBox="0 0 560 165" role="img" aria-label="실력 인지도 협업 변화 그래프"><path d="M20 20V145H540" class="ability-axis"/>${metrics.map(([k,label])=>`<path class="ability-${k}" d="${line(k)}" fill="none" stroke-width="2.5"/><circle class="ability-${k}" cx="${records.length>1?540:20}" cy="${145-records.at(-1)[k]*1.25}" r="3"/>`).join('')}<text x="20" y="160" fill="currentColor" font-size="10">${E.weekDate(records[0].week)}</text><text x="540" y="160" fill="currentColor" font-size="10" text-anchor="end">${E.weekDate(records.at(-1).week)}</text></svg><div class="ability-records">${[...records].reverse().map(r=>`<article><strong>${E.weekDate(r.week)}</strong><span>실력 ${r.skill} · 인지도 ${r.star} · 협업 ${r.coop}</span><small>${esc(r.reason)}</small></article>`).join('')}</div>`,btn('인물정보로 돌아가기','ability-back',`data-id="${p.id}"`,'primary'));}
...
function helpDialog(){const rows=[
 ...
 ['09 · 매출과 정산','새 영화는 평균 결제액 9,800원과 일부 특수상영 가산을 적용합니다. 매출에서 부가세·극장 몫·배급수수료·원작료를 차감한 뒤 제작비를 회수하고 순이익을 배분합니다. 우리 회사 손익은 실제 정산금과 외부 투자금을 더하고 작품 지출을 뺀 값입니다. 재무 및 영화 상세의 수익 정산에서 내역을 확인하세요. 최소 8주 상영 후 주간 관객이 첫 주의 12% 미만 또는 2,500명 미만이면 상영이 끝납니다.'],
 ...
 ['11 · 자금과 시상식','대출은 10억 단위, 월 이자율은 0.5%입니다. 보통 난이도 기본 운영비는 월 0.25억이며 난이도에 따라 80~120%로 조정합니다. 시설비·세금 적립 및 이자는 4주마다 한 번만 결산합니다. 12월 4주 결산 후 그해 개봉작을 대상으로 연말 시상식을 개최합니다.'],
 ...
 ['16 · 자동 저장','저장은 현재 사이트의 이 브라우저에만 남습니다. 다른 기기나 주소와 자동 공유하지 않습니다. 첫 화면 복귀는 기록을 지우지 않지만, 새 게임 확인이나 브라우저의 사이트 데이터 삭제는 기존 기록을 지웁니다. 기획 초안은 자동 보관합니다. 더보기의 저장 관리에서 슬롯·백업·JSON 이동을 사용하고, 기획 보관함에서는 여러 기획을 직접 저장할 수 있습니다.']
...
function businessPanel(f){return `<div class="divider"></div><div class="business-summary">${posterArt(f)}<div><h3>포스터 · 홍보 · 제작 계약</h3><p class="small muted mt-sm">관객 관심 ${plus(f.publicity??0)}% · 홍보 ${(f.promotions??[]).length}회 · 카메오 ${(f.cameos??[]).map(id=>esc(PERSON[id]?.name)).join(', ')||'없음'}</p>${f.script.license?`<p class="small muted mt-sm">${f.script.license.medium} 원작 · 선급 라이선스 ${E.money(f.script.license.fee)} 포함 · 계약상 배급 정산액의 ${Math.round(f.script.license.share*100)}% 지급<br>누적 원작료 ${E.money(f.royalties??0)} · 작품 정산은 원작료 차감 후 금액</p>`:''}${f.company==='c0'?btn('제작·홍보 관리','business',`data-id="${f.id}"`,'primary mt-sm'):''}</div></div>${(f.businessHistory??[]).slice(-8).reverse().map(x=>`<p class="small muted mt-sm">${E.date(x.month)} · ${esc(x.text)}</p>`).join('')}`;}
...
function licenseDialog(){return dialogFrame('원작 라이선스','애니 · 만화 · 소설 · 게임의 세계를 영화로',`<p class="small muted">모든 원작은 게임 속 가상 작품입니다. 선급금은 제작 확정 시 제작비에 포함되며, 새 계약은 주간 극장 매출에서 부가세·극장 몫·배급수수료를 제한 금액에 원작 수익배분을 적용합니다. 속편도 동일한 라이선스 조건을 적용합니다.</p><div class="script-grid mt">${X.IPS.map(ip=>`<article class="card"><span class="tag green">${ip.medium}</span><h3 class="mt-sm">${ip.title}</h3><div class="mt-sm">${tags([ip.genre])}</div><p class="small muted mt-sm">선급 ${E.money(ip.fee)} · 제작사 몫의 ${Math.round(ip.share*100)}%</p>${btn('각색 시놉시스 받기','license',`data-id="${ip.id}"`,'ghost mt-sm')}</article>`).join('')}</div>`);}
...
function renderDialog(){growth?.captureDraft();navigation?.schedule();const more=document.querySelector('.mobile-nav [data-action="more"]');if(more){more.classList.toggle('active',ui.modal?.type==='more'||!navs.slice(0,4).some(n=>n[0]===ui.view));more.setAttribute('aria-expanded',String(ui.modal?.type==='more'));}const dialog=$('dialog');const focus=document.activeElement?.id,selection=document.activeElement?.selectionStart,wasOpen=dialog.open;const scrollBody=dialog.querySelector('.dialog-body');const ownScroll=scrollBody&&getComputedStyle(scrollBody).overflowY==='auto';const top=dialogScroll.transition(dialogKey(ui),ownScroll?scrollBody.scrollTop:dialog.scrollTop);
  ...
  const m=ui.modal;let content=growth?.modalMarkup(m)??updates?.modalMarkup(m)??features?.modalMarkup(m)??'';
  ...
  if(m.type==='economy-details')content=economyDialog(m.id);
  ...
  if(m.type==='ability-history')content=abilityHistory(m.id);
  if(m.type==='script-info')content=scriptInfo(m.id);
  ...
  if(m.type==='more')content=dialogFrame('회사 메뉴',esc(E.player(game).name)+' · '+K.difficulty(game).name,`<nav class="nav-modal">${navs.slice(4).map(([v,i,t])=>btn(`${icon(i)} ${t}`,'nav',`data-view="${v}"`,'ghost')).join('')}${btn(`${icon('library')} 저장 관리`,'growth-storage','','ghost')}${btn(`${icon('home')} 저장하고 첫 화면`,'home','','ghost')}${btn(`${icon('bell')} 알림함`,'notifications','','ghost')}${btn(`${icon('help')} 플레이 가이드`,'help','','ghost')}${btn(`버전 기록 · ${V.APP_VERSION}`,'versions','','ghost')}</nav>`,btn('닫기','close','','primary full'));
  ...
  dialog.className=['rights-info','licenses','runtime-edit','studio-stat','tax-details','wizard','picker','business','completion','film','ott-offers','company-info','film-chart','finance-chart','critic-reviews','audience-reviews'].includes(m.type)||m.type.startsWith('growth-')?'wide':'';dialog.dataset.modal=m.type;dialog.innerHTML=content;dialog.setAttribute('aria-labelledby','dialog-title');document.documentElement.classList.add('modal-open');if(!wasOpen)dialog.showModal();
...
function startPlanning(id){
  ...
  if(E.productionSlots(game).length>=3)throw Error('세 개 제작 슬롯을 모두 사용 중입니다.');
  if(!E.available(game,PERSON[p.writer])||E.busyFilm(game,p.writer))throw Error('이 작가는 다른 작품을 제작 중입니다.');
  if(ui.draft&&ui.draft.script.id!==p.id&&!B.initGrowth(game).drafts.some(x=>x.id!=='auto-draft'&&x.draft.script.id===ui.draft.script.id)){B.archiveDraft(game,ui.draft,ui.step);persist();}
...
document.addEventListener('submit',event=>{
  ...
  try{game=E.createGame($('company-name').value,Date.now()>>>0,Number(new FormData(event.target).get('logo')),new FormData(event.target).get('difficulty')??'normal');ui.home=false;ui.view='studio';ui.modal=null;persist();render();window.scrollTo({top:0,behavior:'instant'});toast('회사 설립을 마쳤습니다. 첫 시나리오를 골라 보세요.');}catch(error){toast(error.message,true);}
});
document.addEventListener('click',()=>navigation?.remember(),true);
document.addEventListener('click',event=>{
  ...
  sound.unlock();if(a!=='sound')sound.play('click');if(ui.advancing||navigation?.travelling)return;
  try{
    if(a==='economy-details'){features.deepOpen('economy-details',id||null);return;}
    if(a==='close'||a==='feature-back'){close();return;}
    if(a==='market-forecast'){ui.market='schedule';go('market');return;}
    ...
    if(growth.handleAction(a,id,button))return;
    ...
    if(a==='reset'){try{localStorage.removeItem(SAVE_KEY)}catch{}game=null;ui.home=false;ui.modal=null;ui.draft=null;ui.posterChoices={};ui.popupTimers.forEach(clearTimeout);ui.popupTimers.clear();invalidSave=false;ui.peekStack=[];navigation?.reset();render();window.scrollTo({top:0,behavior:'instant'});return;}
  }catch(error){toast(error.message||'선택을 다시 확인해 주세요.',true);}
});
function fieldChanged(event){if(growth.handleField(event)||updates.handleField(event)||features.handleField(event))return;const target=event.target;
...
document.addEventListener('change',event=>{if(event.target.id==='difficulty'){const level=K.DIFFICULTY[event.target.value]??K.DIFFICULTY.normal;$('difficulty-capital').textContent=level.capital+'억 원';$('difficulty-hint').textContent=level.description;startScreen.update?.();return;}if(event.target.matches('select,input[type=checkbox]'))fieldChanged(event)});
...
function catalogLayout(view){
 const cached=ui.catalogLayouts?.[view];if(cached)return cached;
 const width=Math.max(280,innerWidth-(innerWidth>1000?254:28));
 const columns=view==='talents'?Math.max(2,Math.min(7,Math.floor(width/172))):Math.max(1,Math.min(3,Math.floor(width/295)));
 return {columns,rows:1,size:columns};
}
// Match the mobile dock breakpoint, including a phone rotated into landscape.
function mobileScriptList(){return window.matchMedia('(max-width:900px), (max-height:500px) and (orientation:landscape)').matches;}
let catalogFrame=0;
function catalogPrepare(){
 const scrolling=!!game&&!ui.home&&ui.view==='scripts'&&mobileScriptList();
 const enabled=!!game&&!ui.home&&(ui.view==='talents'||(ui.view==='scripts'&&!scrolling));
 document.documentElement.classList.toggle('scripts-mobile-scroll',scrolling);
 document.documentElement.classList.toggle('catalog-fit',enabled);
 const v=window.visualViewport;const h=v&&v.scale===1?v.height:innerHeight;
 document.documentElement.style.setProperty('--catalog-viewport',Math.floor(h)+'px');
 document.documentElement.classList.toggle('catalog-short',enabled&&h<510);
}
function fitCatalog(){
 cancelAnimationFrame(catalogFrame);catalogFrame=requestAnimationFrame(()=>{
  ...
  const view=ui.view,short=height<(view==='talents'?158:235);
  document.querySelector('.catalog-screen').classList.toggle('compact-catalog',short);
  const columns=view==='talents'?Math.max(2,Math.min(7,Math.floor((width+8)/(innerWidth<620?155:184)))):Math.max(1,Math.min(3,Math.floor((width+8)/295)));
  const rows=Math.max(1,Math.min(view==='talents'?5:2,Math.floor((height+8)/ (view==='talents'?150:245))));
  const next={columns,rows,size:columns*rows},old=ui.catalogLayouts?.[view];
  if(!old||old.columns!==columns||old.rows!==rows){ui.catalogLayouts??={};ui.catalogLayouts[view]=next;render();}
 });
}
window.addEventListener('resize',()=>{catalogPrepare();fitCatalog();});
window.visualViewport?.addEventListener('resize',()=>{catalogPrepare();fitCatalog();});
document.fonts?.ready.then(()=>fitCatalog());
...
function handleNewAction(a,id,button){
 if(a==='home'){growth.captureDraft(true);if(game)persist();ui.home=true;ui.modal=null;render();window.scrollTo(0,0);return true;}
 if(a==='resume'){
...
features=createStudioFeatures({getGame:()=>game,ui,E,X,R,esc,btn,icon,tags,avatar,dialogFrame,pageHead,open,render,renderDialog,mutate,go,persist,toast,relationBadge,timelinePanel,filmCareerNotes,posterPicker,statusLabel,growth:()=>growth});
...
function installSave(candidate){
  // Only validated imports reach here; a caller saves the prior company in the safety slot first.
  const next=E.migrateSave(candidate);
  localStorage.setItem(SAVE_KEY,JSON.stringify(next)); // fail without replacing memory if active storage is full
  game=next;ui.home=false;ui.view='studio';ui.modal=null;ui.peekStack=[];ui.draft=null;ui.step=0;
  ui.pendingSelection=null;ui.replacement=null;ui.picker=null;ui.posterChoices={};
  ui.popupTimers.forEach(clearTimeout);ui.popupTimers.clear();dialogScroll.clear();
  growth.restoreAuto();navigation?.reset();persist();render();window.scrollTo(0,0);
}
growth=createGrowthFeatures({getGame:()=>game,ui,E,esc,btn,icon,avatar,dialogFrame,pageHead,render,renderDialog,persist,toast,features,statusLabel,startPlanning,installSave});
growth.restoreAuto();
window.addEventListener('pagehide',()=>growth.captureDraft(true));
document.addEventListener('input',()=>growth.captureDraft());
...
document.addEventListener('keydown',e=>{
 ...
 if(game&&!ui.home&&!ui.advancing){
  if(!ui.modal)target=[...document.querySelectorAll('[data-action="advance"]')].find(b=>b.getClientRects().length);
...
function navigationScreen(){
 ...
 return cloneNavigationState({home:!game||ui.home,view:ui.view,modal:ui.modal?{...ui.modal}:null,state,draftId:ui.draft?.script?.id??null,scroll:{page:window.scrollY,dialog:body?.scrollTop??0,dialogOuter:$('dialog').scrollTop,filters:document.querySelector('#main .filter-controls')?.scrollLeft??0}});
}
function navigationHome(){return {...navigationScreen(),home:true,view:'studio',modal:null,state:{...navigationScreen().state,peekStack:[]},scroll:{page:0,dialog:0,dialogOuter:0,filters:0}};}
function restoreNavigation(screen){
 const supported=new Set(navs.map(n=>n[0]));
 ui.home=!game||!!screen.home;ui.view=supported.has(screen.view)?screen.view:'studio';
 for(const k of navigationFields){if(screen.state?.[k]!==undefined)ui[k]=cloneNavigationState(screen.state[k]);else if(!['people','step','market','filmTab','businessTab','scriptGenre'].includes(k))delete ui[k];}
 ui.peekStack??=[];ui.modal=screen.modal?{...screen.modal}:null;
 ...
 if(game&&filmTypes.includes(type)){
  ...
  if(!f){ui.modal=null;ui.peekStack=[];}
  else if(type==='event'&&f.pending==null||type==='completion'&&f.completionAcknowledged||['ott-offers','ott-confirm'].includes(type)&&!['ready','shelved'].includes(f.status)||type==='runtime-edit'&&!E.pendingEdits(game).some(x=>x.id===f.id)) {ui.modal={type:'film',id:f.id};ui.filmTab='overview';ui.peekStack=[];}
 }
 if(game&&['person','ability-history'].includes(type)&&!E.person(game,ui.modal?.id)){ui.modal=null;ui.peekStack=[];}
 render();
 const restoreScroll=()=>{if(!ui.home)window.scrollTo({top:screen.scroll?.page??0,behavior:'instant'});const body=$('dialog').querySelector('.dialog-body');if(body)body.scrollTop=screen.scroll?.dialog??0;$('dialog').scrollTop=screen.scroll?.dialogOuter??0;const filters=document.querySelector('#main .filter-controls');if(filters)filters.scrollLeft=screen.scroll?.filters??0;};
 restoreScroll();requestAnimationFrame(()=>requestAnimationFrame(restoreScroll));
}
navigation=createNavigation({read:navigationScreen,restore:restoreNavigation,home:navigationHome});

render();navigation.start();registerPageTools();


function economyDialog(id){
 const level=K.difficulty(game),film=id?game?.films.find(x=>x.id===id):null;
 const money=n=>E.money(n??0),percent=n=>Math.round(n*100)+'%';
 const values=rows=>`<dl class="economy-values">${rows.map(([label,value])=>`<div><dt>${label}</dt><dd>${value}</dd></div>`).join('')}</dl>`;
 let body='';
 if(film){
  if(!K.modernFilm(film))body=`<div class="notice"><p>이 영화는 업데이트 전 계약입니다. 기존 티켓·투자 배분 방식과 과거 정산을 유지합니다. 새로 계약하는 영화부터 새 기준을 적용합니다.</p></div>`;
  else{
   const e=film.economy,t=e.theatrical,i=film.investment;
   body=`<h3>극장 매출 → 작품 정산</h3>${values([
    ['누적 티켓 매출',money(t.gross)],['부가세 제외분',money(t.vat)],['극장 배분',money(t.exhibitor)],['배급수수료',money(t.distribution)],['원작 수익배분',money(t.royalty)],['비용 회수에 들어오는 극장 정산',money(t.pool)]
   ])}<h3 class="mt">극장·OTT·해외 합산 비용 회수</h3>${values([
    ['총 작품 지출',money(film.spent)],['누적 비용 회수',money(e.returnedCost)],['아직 회수하지 못한 비용',money(Math.max(0,film.spent-e.returnedCost))],['비용 회수 후 누적 순이익 풀',money(e.profitPool)],['제작사 실제 누적 입금',money(film.receipts)],['제작사 작품 손익',money(B.filmProfit(film))]
   ])}${i?`<h3 class="mt">${esc(i.name)} 정산</h3>${values([
    ['투자금',money(i.amount)],['투자 원금 회수',money(i.principalRecovered)],['투자사 순이익 배분율',percent(i.share)],['투자사 누적 순이익',money(i.profitPaid)],['투자사 총 수령액',money(i.paid)]
   ])}`:'<p class="small muted">자체 제작: 비용 회수액과 남은 순이익은 모두 제작사에 귀속됩니다.</p>'}<div class="notice small"><p>남은 작품비를 극장에서만 회수하려면 약 <strong>${E.viewers(K.breakEvenAudience(film,C.formatEffect(film).ticketFactor))}명</strong>의 추가 관객이 필요합니다. 단가·상영 비율을 고정한 추정이며 운영비·법인세와 미래 추가 비용은 제외합니다.</p></div>`;
  }
 }
 body+=`<details class="fold-section" ${film?'':'open'}><summary>대한민국 시장을 참고한 게임 정산 기준</summary><p>새 영화의 기본 평균 결제금액은 9,800원입니다. 일반권·할인권의 평균을 가정하며 특수상영은 일부 좌석의 가중치만 더합니다.</p><p>티켓 매출 ÷ 1.1 → 극장 50% / 영화 측 50% → 영화 측 금액의 배급수수료 10% → 원작료 → 제작·홍보 비용 회수 → 순이익 배분 순서입니다.</p><p>공동제작은 실제 투자액을 한도로 먼저 원금을 비례 회수합니다. 그 뒤 순이익 중 투자자 측 60%에 외부 투자 비율을 곱해 외부 투자사 몫을 정합니다. 제작사에는 제작 지분 40%와 자기 투자 지분이 남습니다.</p><p>출연료는 회당이 아닌 <strong>영화 한 편의 계약료</strong>입니다. 직군·인지도·실력·필모그래피·작품 규모·관계를 고려하며, 확정 뒤 기존 계약금을 소급 변경하지 않습니다.</p><p class="small muted">9,800원은 2025년 영진위의 전체 시장·한국영화 평균을 참고해 정한 게임값입니다. 극장 50:50은 게임 가정, 배급 10%·순이익 60:40은 2014년 표준계약의 협의 가능한 참고 기준이며 모든 계약의 고정 요율은 아닙니다. 실제 배우별 단가·OTT·해외 계약은 공개 자료만으로 확정할 수 없어 게임 계수를 사용합니다.</p><p class="small muted">자금 회전을 위해 주간 정산과 기존 대출·투자 기능을 유지합니다. 실제 부가세 신고·매입세액 환급 및 비용 자산화까지 재현하지 않습니다. 출처와 가정은 동봉한 KOREAN_MARKET_MODEL.md에 구분했습니다.</p></details>`;
 body+=`<details class="fold-section" ${film?'':'open'}><summary>5단계 난이도 · 현재 ${level.name}</summary><p class="small muted">시작할 때 선택하며 현재 저장의 난이도는 고정됩니다. 신규 입장 수요·새 OTT/해외 제안·기본 운영비만 보정하고 평점·시간·기능·세율·계약 배분율은 바꾸지 않습니다.</p><div class="economy-difficulty-list">${K.DIFFICULTIES.map(x=>`<div class="${x.id===level.id?'current':''}"><strong>${x.name}</strong><span>시작 ${x.capital}억</span><small>극장 수요 ${percent(x.demand)} · 판권 ${percent(x.deal)} · 기본 운영비 ${percent(x.overhead)}</small></div>`).join('')}</div></details>`;
 return dialogFrame(film?'작품 수익 정산':'수익 정산 기준 · 난이도',film?esc(film.title):'현실의 구조를 참고하고, 플레이 흐름은 유지합니다.',body,btn('뒤로가기','close','','primary'));
}
