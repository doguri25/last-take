...
function scriptsPage(){
 const query=(ui.scriptQuery??'').trim(),all=game.pitches.filter(p=>(ui.scriptGenre==='all'||p.genre===ui.scriptGenre)&&(!query||[p.title,p.synopsis,PERSON[p.writer]?.name].some(x=>x?.includes(query)))),layout=catalogLayout('scripts'),size=layout.size,pages=Math.max(1,Math.ceil(all.length/size));
...
function catalogPrepare(){
 const enabled=!!game&&!ui.home&&['talents','scripts'].includes(ui.view);
